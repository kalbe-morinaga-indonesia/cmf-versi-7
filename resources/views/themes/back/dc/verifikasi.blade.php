...
@section('title','Verifikasi CMF - CMF Online')

@section('content')
    <h4 class="fw-bold"><span class="text-muted fw-light">CMF / </span> Verifikasi</h4>
            ...
            @if($cmf->step == 1)
                ...
                <div class="card mb-4">
                    <div class="card-header">
                        ...
                        <form action="{{route('cmf.dc.store',['slug' => $cmf->slug])}}" method="post">
                            @csrf
                ...
                <div class="card mb-4">
                    <div class="card-header">
                        ...
                        <form action="{{route('cmf.dc.store',['slug' => $cmf->slug])}}" method="post">
                            ...
                            <div class="form-floating">
                                <select class="form-select @error('verifikasi') is-invalid @enderror" name="verifikasi" id="verifikasi" aria-label="Floating label select example">
                                    <option value="setuju" @selected(old('verifikasi') == 'setuju')>Setuju</option>
                                    <option value="tidak_setuju" @selected(old('verifikasi') == 'tidak_setuju')>Tidak Setuju</option>
                                </select>
                                <label for="verifikasi">Verifikasi</label>
                            ...
                            <button type="submit" class="btn btn-primary">Evaluasi & Verifikasi</button>
                        </form>
                    </div>
                </div>
            @elseif($cmf->step == 8)
                ...
                <div class="card mb-4">
                    <div class="card-header">
                        ...
                        <form action="{{route('cmf.dc.store',['slug' => $cmf->slug])}}" method="post">
                            ...
                            <button type="submit" class="btn btn-primary my-4">Verifikasi</button>
                        </form>
                    </div>
                </div>
            @elseif($cmf->step == 10)
                ...
                <div class="card mb-4">
                    <div class="card-header">
                        <h6 class="card-title">Document Control</h6>
                        <form action="{{route('cmf.dc.store',['slug' => $cmf->slug])}}" method="post">
                            @csrf
                            <div class="form-floating mb-3">
                                <input type="text" class="form-control" name="dc" id="dc" value="{{$dc->name}}" placeholder="DC" readonly>
                                <label for="floatingInput">Document Control</label>
                            </div>
                            <label class="form-label fw-bold">Verifikasi Dokument</label>
                                ...
                                <table class="table table-sm mb-4">
                                    ...
                                    @foreach($documents as $document)
                                        <tr>
                                            <td>
                                                <div class="mb-3">
                                                    <input type="text" class="form-control" name="post_document[]" id="post_document" placeholder="Post Document" value="{{$document->post_document}}" readonly>
                                                </div>
                                            </td>
                                            <td>
                                                <div class="form-floating">
                                                    <select class="form-select" name="applicable[]" id="applicable" aria-label="Floating label select example">
                                                        <option value="applicable" @selected(old('verifikasi') == 'setuju')>applicable</option>
                                                        <option value="not_applicable" @selected(old('verifikasi') == 'tidak_setuju')>Not Applicable</option>
                                                    </select>
                                                    <label for="verifikasi">Verifikasi</label>
                                                </div>
                                            </td>
                    ...
                    @elseif($step->step == 8)
                        <div class="card mb-4">
                                <div class="card-header">
                                    ...
                                    <form action="#">
                                        <div class="form-check">
                                            <input class="form-check-input" type="radio" name="verifikasi" id="setuju" value="setuju" {{$step->step == 8 && $step->is_signature == 1 ? 'checked' : ''}} @if($step->step == 8) disabled @endif>
                                            <label class="form-check-label" for="setuju">
                                                Setuju
                                            </label>
                                        </div>
                                        <div class="form-check">
                                            <input class="form-check-input" type="radio" name="verifikasi" id="tidak_setuju" value="tidak setuju" {{$step->step == 8 && $step->is_signature == 0 ? 'checked' : ''}} @if($step->step == 8) disabled @endif>
                                            <label class="form-check-label" for="tidak setuju">
                                                Tidak Setuju
                                            </label>
                                        </div>
                                        <div class="form-floating my-4">
                                            <textarea class="form-control" name="catatan" placeholder="Leave a note here" id="catatan" @if($step->step == 8) disabled @endif>{{$step->catatan ?? ''}}</textarea>
                                            <label for="catatan">Catatan</label>
                                        </div>
                                        <button type="submit" class="btn btn-primary" @if($step->step == 8) disabled @endif>
                                            {{$step->step == 8 ? 'Sudah Diverifikasi' : 'Verifikasi'}}</button>
                                    </form>
                                </div>
                            </div>
                    @elseif($step->step == 9)
                        <div class="card mb-4">
                            <div class="card-header">
                                <h6 class="card-title">Depthead {{$step->user->department->txtNamaDept}}</h6>
                                <form action="#">
                                    <div class="form-check">
                                        <input class="form-check-input" type="radio" name="verifikasi" id="setuju" value="setuju" {{$step->step == 9 && $step->is_signature == 1 ? 'checked' : ''}} @if($step->step == 9) disabled @endif>
                                        <label class="form-check-label" for="setuju">
                                            Setuju
                                        </label>
                                    </div>
                                    <div class="form-check">
                                        <input class="form-check-input" type="radio" name="verifikasi" id="tidak_setuju" value="tidak setuju" {{$step->step == 9 && $step->is_signature == 0 ? 'checked' : ''}} @if($step->step == 9) disabled @endif>
                                        <label class="form-check-label" for="tidak setuju">
                                            Tidak Setuju
                                        </label>
                                    </div>
                                    <button type="submit" class="btn btn-primary" @if($step->step == 3) disabled @endif>
                                        {{$step->step == 9 ? 'Sudah Diverifikasi' : 'Verifikasi'}}</button>
                                </form>
                            </div>
                        </div>
                    @elseif($step->step == 10)
                        <div class="card mb-4">
                            <div class="card-header">
                                ...
                                <form action="#">
                                    <div class="form-check">
                                        <input class="form-check-input" type="radio" name="verifikasi" id="setuju" value="setuju" {{$step->step == 10 && $step->is_signature == 1 ? 'checked' : ''}} @if($step->step == 10) disabled @endif>
                                        <label class="form-check-label" for="setuju">
                                            Setuju
                                        </label>
                                    </div>
                                    <div class="form-check">
                                        <input class="form-check-input" type="radio" name="verifikasi" id="tidak_setuju" value="tidak setuju" {{$step->step == 10 && $step->is_signature == 0 ? 'checked' : ''}} @if($step->step == 10) disabled @endif>
                                        <label class="form-check-label" for="tidak setuju">
                                            Tidak Setuju
                                        </label>
                                    </div>
                                    <button type="submit" class="btn btn-primary my-3" @if($step->step == 10) disabled @endif>
                                        {{$step->step == 10 ? 'Sudah Diverifikasi' : 'Verifikasi'}}</button>
                                </form>
                            </div>
                        </div>
                @elseif($step->step == 11)
                    <div class="card mb-4">
                        <div class="card-header">
                            ...
                            <form action="#">
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="verifikasi" id="setuju" value="setuju" {{$step->step == 11 && $step->is_signature == 1 ? 'checked' : ''}} @if($step->step == 11) disabled @endif>
                                    <label class="form-check-label" for="setuju">
                                        Setuju
                                    </label>
                                </div>
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="verifikasi" id="tidak_setuju" value="tidak setuju" {{$step->step == 11 && $step->is_signature == 0 ? 'checked' : ''}} @if($step->step == 11) disabled @endif>
                                    <label class="form-check-label" for="tidak setuju">
                                        Tidak Setuju
                                    </label>
                                </div>
                                <button type="submit" class="btn btn-primary my-4" @if($step->step == 11) disabled @endif>
                                    {{$step->step == 11 ? 'Sudah Diverifikasi' : 'Verifikasi'}}</button>
                            </form>
                        </div>
                    </div>
                @endif
